...
func init() {
  ...
  rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", 
    "config file (default is $HOME/.go-pomodoro.yaml)")
  rootCmd.Flags().DurationP("go-pomodoro", "p", 25*time.Minute,
    "Pomodoro duration")
  ...
  viper.BindPFlag("long", rootCmd.Flags().Lookup("long"))
	...
	// rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.go-pomodoro.yaml)")
...
func initConfig() {
  if cfgFile != "" {
    ...
    viper.SetConfigName(".go-pomodoro")
  }
