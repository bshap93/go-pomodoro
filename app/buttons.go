...
// add function newButtonSet to instantiate buttonSet
// take a context, a pointer to pomodoro.ItervalConfig, a pointer to widgets,
// redrawCh channel and a errorCh
func newButtonSet(ctx context.Context, config *pomodoro.IntervalConfig,
	...
	pauseInterval := func() {
		...
		if err := i.Pause(config); err != nil {
			if err == pomodoro.ErrIntervalsNotRunning {
				return
			}
			errorCh <- err
			return
